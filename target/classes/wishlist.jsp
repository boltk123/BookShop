	...
	<div class="container">
		...
		<ul id="charactersList">
			<c:forEach var="item" items="${sessionScope.wishlist}">
				<li class="character">
					<a href="WishList?id=${item.book_id}">
						<img src="data:image/jpg;base64,${item.base64Image}" alt="">
						<div style="display: flex; justify-content: space-between; align-items: center;">
							<div>
								<h2>${item.title}</h2>
								<p>${item.genre}</p>
							</div>
							<a href="wishlist?action=remove&hotelId=${item.book_id}" class="btn"><i style="font-size: 30px;"class="fas fa-heart"></i></a>
						</div>
					</a>
				</li>
			</c:forEach>
